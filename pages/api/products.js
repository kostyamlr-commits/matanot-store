...
export default async function handler(req, res) {
  ...
  try {
    const page=parseInt(req.query.page)||1, limit=parseInt(req.query.limit)||6
    ...
    else if (cat==='new') query=query.order('created_at',{ascending:false})
    else if (cat) query=query.eq('cat_tag',cat)
    if (cat!=='new') query=query.order('orders',{ascending:false})
